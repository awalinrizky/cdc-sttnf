...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { data: job, error } = await supabase.from("jobs").select("id, title, company_name, location, description, apply_url, is_active, created_at").eq("id", id).eq("is_active", true).single();
  if (!job || error) {
